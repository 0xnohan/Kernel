...
class Miner:
    ...
    def mine_block_thread(self, block_header, block_height, transactions):
        ...
        block_size = len(mined_header.serialize()) + sum(len(tx.serialize()) for tx in transactions)
        new_block = Block(block_height, block_size, mined_header,len(transactions), transactions)
        submission_payload = {"command": "submit_block", "params": {"block_hex": new_block.serialize().hex()}}
        ...
        if submission_response and submission_response.get('status') == 'success':
            print(f"Successfully submitted block {block_height}. Hash: {mined_header.generateBlockHash()}")
        else:
            message = submission_response.get('message', 'Unknown error') if submission_response else "No response"
    ...
    def run(self):
        ...
        while True:
            ...
            print(f"New work received for block height {template['height']}. Starting mining...")
