...
def start_process_in_new_terminal(script_path, process_key):
    ...
    if sys.platform == "win32":
        ...
        process = subprocess.Popen(
            ["gnome-terminal", "--", sys.executable, script_path], cwd=current_dir
        )
    else:
        logger.error(
...
def SendRpcCommand(host, rpc_port, command):
    try:
        ...
        return {
            "status": "error",
            "message": "No connection. Please check if the daemon is running",
        }
...
def settings(host, rpc_port):
    while True:
        ...
        if choice == "1":
            ...
            wallets_response = SendRpcCommand(
                host, rpc_port, {"command": "get_wallets"}
            )
            if wallets_response.get("status") == "success" and wallets_response.get(
                "wallets"
            ):
                ...
                try:
                    wallet_choice_str = input(
                        f"Select a wallet (current: {c_wallet}): "
                    )
        ...
        if choice == "1":
            ...
            if wallets_response.get("status") == "success" and wallets_response.get(
                ...
                for i, wallet in enumerate(wallets):
                    ...
                    print(
                        f"  {i + 1} - {wallet.get('WalletName')} ({wallet.get('PublicAddress')}) - Balance: {balance:.8f} KOR"
                    )
